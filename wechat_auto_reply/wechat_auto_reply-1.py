...
def write(text):
    ...
    # 直接全部复制粘贴，没有灵魂，没有那种输入效果，一个一个复制粘贴效果好一些，但输入就会慢很多，每个字符停一秒，复制粘贴还是会出错
    # for word in text:
    # 	pyperclip.copy(word)  # 复制文本到粘贴板
    # 	time.sleep(1)
...
def open_wechat():
    ...
    if wechat_window_location:
        ...
        print('切换到桌面')
        pyautogui.hotkey('win', 'd')  # 切换到桌面
        wechat_desktop_shortcut_image = 'images/wechat_desktop_shortcut.png'  # 微信应用
        wechat_desktop_shortcut_location = pyautogui.locateCenterOnScreen(wechat_desktop_shortcut_image, confidence=0.85, minSearchTime=2)
        if wechat_desktop_shortcut_location:
            print('打开微信')
